...
## Re-running Steps and Resetting Data

Each step's re-run behaviour:
...
```bash
sqlite3 "data/<name>_rag.db" "DELETE FROM sections; DELETE FROM raw_blocks; UPDATE documents SET docling_json = NULL; DELETE FROM parent_chunks;"
```
Step0 will still skip (document stays registered), which is correct.
...
`section_header` added to `NOISE_BLOCK_TYPES` (`text_normalizer.py`).
`build_section_content()` (`chunker.py`) now explicitly prepends `## heading`
from `sections` table metadata for the level-2 main section, and all levels
use the same pattern. Heading construction is fully decoupled from raw block
content at all section levels.

### Figure/diagram text fragments in body content (unfixed)

Docling extracts text labels from inline process diagrams (step numbers "1" "2"
"3", box labels like "WAREHOUSING", "COMMUNITY") as standalone `text` blocks
indistinguishable from body text. These pollute parent chunks.
